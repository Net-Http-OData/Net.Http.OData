...
namespace Net.Http.OData.Tests
{
    public class ODataErrorContentSerializationTests_NewtonsoftJsonNet
    {
        ...
        public void JsonSerializationWith_Code_Message()
        {
            ...
            string jsonResult = JsonConvert.SerializeObject(odataErrorContent, TestHelper.JsonSerializerSettings);

            Assert.Equal("{\"error\":{\"code\":\"404\",\"message\":\"Not Found\"}}", jsonResult);

            ODataErrorContent deserializedODataErrorContent = JsonConvert.DeserializeObject<ODataErrorContent>(jsonResult, TestHelper.JsonSerializerSettings);
